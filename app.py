...
ADMIN_PASS_ENV = os.environ.get("ADMIN_PASSWORD") # À configurer sur Render
...
<head>
    ...
    <style>
        ...
        /* NAVIGATION FLUIDE IPHONE style */
        .nav-container {
            background: rgba(0,0,0,0.5); border-bottom: 0.5px solid #222;
            padding: 12px 0;
        }
        .nav-cats { 
        ...
        .nav-cats::-webkit-scrollbar { display: none; } /* Cache la barre moche */

        .cat { 
            color: #8e8e93; text-decoration: none; font-size: 0.95rem; font-weight: 600; 
            padding: 8px 18px; white-space: nowrap; border-radius: 20px;
            background: var(--gray); transition: all 0.3s cubic-bezier(0.4, 0, 0.2, 1);
        }
        .cat.active { 
            color: #fff; background: linear-gradient(135deg, var(--blue), var(--purple));
            transform: scale(1.05); box-shadow: 0 4px 15px rgba(0, 210, 255, 0.3);
        }
        ...
        .card { 
            ...
            margin-bottom: 25px; border: 0.5px solid #333;
            transition: transform 0.2s ease;
        }
        .card:active { transform: scale(0.98); } /* Effet pression iPhone */

        .card-img { width: 100%; height: 250px; object-fit: cover; }
        
        .card-body { padding: 20px; }
        .card-tag { color: var(--blue); font-size: 11px; font-weight: 700; text-transform: uppercase; letter-spacing: 1px; }
        .card-title { margin: 8px 0; font-size: 20px; font-weight: 700; line-height: 1.2; }
        ...
        .btn { 
            ...
            text-decoration: none; font-weight: 700; margin-top: 15px; border:none; width:100%;
        }

        input, textarea, select { 
            width: 100%; padding: 15px; margin: 10px 0; background: #2c2c2e; 
            border: none; color: #fff; border-radius: 12px; font-size: 16px; 
        }
    </style>
</head>
<body>
...
def read_article(id):
    ...
    content = f'<div class="container"><img src="{art.get("img_url") or LOGO_URL}" style="width:100%; border-radius:24px; margin-bottom:20px;"><h1 style="font-size:28px;">{art.get("titre")}</h1><div style="white-space:pre-wrap; font-size:18px; color:#d1d1d6;">{art.get("texte")}</div><br><a href="/" class="btn" style="background:#3a3a3c;">RETOUR</a></div>'
